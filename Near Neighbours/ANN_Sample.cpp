...
int main1(int argc, char **argv)
{
	...
	while (readPt(*queryIn, queryPt)) {			// read query points
        ...
        kdTree->annkFRSearch(queryPt, r, k, nnIdx, dists);
		/*
    ...
    cout << "Total number of edges: " << totalNum << endl;
...
void getArgs(int argc, char **argv)
{
	...
	if (argc <= 1) {							// no arguments
		cerr << "Usage:\n\n"
		<< "  ann_sample [-d dim] [-max m] [-nn k] [-e eps] [-r rad] [-df data]"
        " [-qf query]\n\n"
		<< "  where:\n"
